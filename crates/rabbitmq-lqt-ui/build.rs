...
fn build_wasm(manifest_dir: &Path, frontend_dir: &Path) {
    let wasm_crate_dir = manifest_dir.join("../rabbitmq-lqt-ql-wasm");
    let wasm_output_dir = frontend_dir.join("src/wasm/pkg");
    let wasm_file = wasm_output_dir.join("rabbitmq_lqt_ql_wasm_bg.wasm");
    ...
    let ql_core_dir = manifest_dir.join("../rabbitmq-lqt-ql-core/src");
    ...
    // or re-run it after changing rabbitmq-lqt-ql-core or rabbitmq-lqt-ql-wasm sources.
...
fn main() {
    ...
    if dist_dir.exists() && dist_dir.join("index.html").exists() {
        ...
        let wasm_pkg = frontend_dir.join("src/wasm/pkg/rabbitmq_lqt_ql_wasm_bg.wasm");
        if wasm_pkg.exists() {
            return;
        }
    }
