...
fn test_maintenance_mode_entering() {
    let entry = create_test_entry("Entering maintenance mode", Severity::Info);
    let labels = annotate_labels(&entry);
    assert!(labels.contains(LogEntryLabels::MAINTENANCE_MODE));
}

#[test]
fn test_maintenance_mode_resetting_status() {
    let entry = create_test_entry("Resetting node maintenance status", Severity::Info);
    let labels = annotate_labels(&entry);
    assert!(labels.contains(LogEntryLabels::MAINTENANCE_MODE));
}
...
fn test_maintenance_mode_marking() {
    let entry = create_test_entry("Marking the node as undergoing maintenance", Severity::Info);
    let labels = annotate_labels(&entry);
    assert!(labels.contains(LogEntryLabels::MAINTENANCE_MODE));
}
