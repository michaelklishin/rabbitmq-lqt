...
fn test_federation_queue() {
    let entry = create_test_entry("Starting Federation queue link", Severity::Info);
...
fn test_disconnecting_from_queue() {
    let entry = create_test_entry(
        "Federation link disconnecting from queue 'test'",
        Severity::Info,
    );
    let labels = annotate_labels(&entry);
    assert!(labels.contains(LogEntryLabels::QUEUE_FEDERATION));
...
#[test]
fn test_no_match_regular_queue() {
    let entry = create_test_entry("Regular queue operation", Severity::Info);
    let labels = annotate_labels(&entry);
    assert!(!labels.contains(LogEntryLabels::QUEUE_FEDERATION));
}
