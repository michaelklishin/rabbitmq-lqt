...
use rabbitmq_lqt_lib::entry_metadata::label_annotators::{LabelAnnotator, ShovelsAnnotator};
use rabbitmq_lqt_lib::entry_metadata::labels::LogEntryLabels;

#[test]
fn test_shovels_annotator_matches_connected() {
...
#[test]
fn test_shovels_annotator_matches_in_vhost() {
    let entry = create_test_entry(
        "Shovel 'test-shovel' in virtual host '/' started",
        Severity::Info,
    );
    let annotator = ShovelsAnnotator;
    assert!(annotator.does_match(&entry));
...
#[test]
fn test_shovels_annotator_no_match() {
...
#[test]
fn test_shovels_annotator_annotates() {
    let annotator = ShovelsAnnotator;
