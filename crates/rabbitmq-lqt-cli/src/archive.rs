...
use std::io::{BufRead, BufReader, Error as IoError, ErrorKind, Read, Result as IoResult};
...
impl XzReader {
    pub fn new(mut file: File) -> IoResult<Self> {
        let mut compressed = Vec::new();
        file.read_to_end(&mut compressed)?;

        let mut decompressed = Vec::new();
        lzma_rs::xz_decompress(&mut compressed.as_slice(), &mut decompressed)
            .map_err(|e| IoError::new(ErrorKind::InvalidData, format!("XZ error: {}", e)))?;
...
impl Read for XzReader {
    fn read(&mut self, buf: &mut [u8]) -> IoResult<usize> {
        ...
        let to_read = min(buf.len(), remaining.len());
