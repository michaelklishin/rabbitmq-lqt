...
use rlqt_lib::entry_metadata::subsystem_annotators::{MqttPluginAnnotator, SubsystemAnnotator};
...
fn test_mqtt_retained_message_store() {
    ...
    let annotator = MqttPluginAnnotator;
...
fn test_annotate_sets_subsystem() {
    ...
    assert_eq!(entry.subsystem_id, Some(Subsystem::MqttPlugin.to_id()));
}
