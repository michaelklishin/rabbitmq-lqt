...
use rlqt_lib::Subsystem;
use rlqt_lib::entry_metadata::LABEL_NAMES;
use rlqt_ql::ast::Field;
use rlqt_ql::autocomplete::{
...
fn test_labels_match_canonical_source() {
    for label in LABEL_NAMES {
        assert!(
            ...
            "Label '{}' from rlqt-lib is missing in autocomplete LABELS",
            label
        );
    }
    ...
    for label in LABELS {
        assert!(
            ...
            "Label '{}' in autocomplete LABELS is not in rlqt-lib's canonical list",
            label
        );
    }
}
...
fn test_subsystems_match_canonical_source() {
    for subsystem_str in SUBSYSTEMS {
        assert!(
            ...
            "Subsystem '{}' in autocomplete is not a valid rlqt-lib Subsystem",
            subsystem_str
        );
    }
}
